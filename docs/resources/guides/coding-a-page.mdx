...
import { Container, styled } from "@washingtonpost/wpds-ui-kit";
...
WPDS ships with a responsive Container component that centers its content and resizes based on window width. It has four sizes based on UI Kit's internal breakpoints. For small and medium breakpoints its width is 100%, large its width is 900px, extra large and extra extra large 1024px, and above extra large 1440px. In the initial stage of developing the layout we'll use the [stitches `styled` function](https://stitches.dev/docs/api#styled) that ships with UI Kit to make the container easier to visualize.
...
By default Container is set to display flex with a direction of column, our layout adds two children. A Header to contain the elements that span the width of the page, and a OverviewDetail to contain the grid of recipe photos and the selected recipe's details. StyledContainer un-sets alignItems to allow its child elements to have a 100% width. It also sets a flex property of 1 on OverviewDetail using an nth-child selector to allow it to expand and fill any vertical space not occupied by the Header. Using a child selector keeps OverviewDetail separated from the concern of how it is layed out in its parent.

The OverviewDetail has its display property set to `flex` to layout its child elements in a row.The Overview element also uses `flex: 1` to expand and fill the main portion of OverviewDetail's flex row. Finally the Detail element starts out in it's large screen layout, using flex's [two value syntax](https://developer.mozilla.org/en-US/docs/Web/CSS/flex#syntax) to set its width to 25%.

#### Adding Responsiveness

In the proposed layout Details responds to screen size changes and will display as an overlay on smaller screen. We'll use a combination of variants and breakpoints in stitches to achieve this.
...
To selectively apply a style a prop with the parent variant name on the element is passed on object that contains keys of breakpoint names and values of child variant names. An initial variant can be set by using the `@initial` key.
...
To keep this guide self contained we will be working with a local data source, but the approach for interacting with an API will be the similar in a production environment.
...
Which can can then be used to populate the grid and the details pane.
...
Then we'll add a couple additional components to properly display the recipes.
...
OverviewDetail has some additional responsibilities in the Detail component. If no children are passed it won't render, facilitating it's show hide functionality. It also now contains a conditionally rendered divider that is hidden when it displays as an overlay.
...
```jsx
import { Button, Icon, styled } from "@washingtonpost/wpds-ui-kit";
...
const RecipeCard = ({ content, onClick }) => {
  ...
  return (
    <Card>
      ...
      >
        {isFavorite ? (
          <Icon size="100" fill={"$red200"} label="Remove from favorites">
            <LikeSolid />
          </Icon>
        ) : (
...
Recipe is a straightforward display of the image and recipe text content broken out by type. A small bit of logic is used for parsing out in ingredients into a list.
...
In order to animate the transition of the Detail pane we'll leverage a [react-transition-group CSSTransition](https://reactcommunity.org/react-transition-group/css-transition) The CSSTransition takes care of the conditional rendering by using the `mountOnEnter` and `unmountOnExit` flags. Since the root application determines of a Recipe is being rendered a copy pf that element is kept and rendered to show while the exit transition is happening.

The standard class naming pattern is use to apply a fade in and out any time Details is shown and hidden, while a slide in is used on the larger side column layout. The `&` sign is used to chain the react-transition-group class names in stitches and apply the rules for the transitions
