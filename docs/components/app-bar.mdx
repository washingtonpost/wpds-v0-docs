...
The anatomy of an app bar is that it has no visual elements but rather it is an empty container that has a built-in utility to make positioning on a layout easier.
...
The following positions are available `sticky`, `relative`, `absolute`, and `fixed`. The behavior of the different options are described in the [MDN web docs](https://developer.mozilla.org/en-US/docs/Web/CSS/position).
