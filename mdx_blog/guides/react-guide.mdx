---
title: How to use WPDS's React UI Kit
...
We're going to build a simple "save to reading list" feature for the website.
...
export const SaveToList = () => {
	return (
		<Icon size="32" label="Save to reading list">
...
We will use a new asset from our [Assets Manager](/foundations/assets). We will use the `Add` asset and apply the `fill` prop to set the color.

```jsx
<Add fill={theme.colors.onPrimary} />
```

Let's create a new button component with the following code:
